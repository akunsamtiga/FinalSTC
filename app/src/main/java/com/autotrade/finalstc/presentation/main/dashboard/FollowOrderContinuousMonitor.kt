...
class FollowOrderContinuousMonitor(
    private val scope: CoroutineScope,
    private val getUserSession: suspend () -> UserSession?,
    private val onFollowTradeResult: (String, Boolean, Map<String, Any>) -> Unit,
    private val serverTimeService: ServerTimeService?
) {
    companion object {
        private const val TAG = "FollowOrderMonitor"
        ...
        private const val FOLLOW_ORDER_TIMEOUT = 90000L
    ...
    private val activeFollowOrders = mutableMapOf<String, FollowOrderMonitoringState>()
    ...
    data class FollowOrderMonitoringState(
        val followOrderId: String,
        val trend: String,
        val amount: Long,
        val assetRic: String,
    ...
    fun startMonitoring() {
        if (isMonitoringActive) {
            Log.d(TAG, "Follow Order monitoring already active")
            return
        }

        isMonitoringActive = true
        Log.d(TAG, "Starting ultra-fast Follow Order monitoring")
    ...
    fun stopMonitoring() {
        ...
        isPreWarmed = false
        Log.d(TAG, "Follow Order monitoring stopped")
    }

    fun addFollowOrderForMonitoring(
        ...
        scope.launch {
            monitoringMutex.withLock {
                ...
                val monitoringState = FollowOrderMonitoringState(
                ...
                activeFollowOrders[followOrderId] = monitoringState

                Log.d(TAG, "Added Follow Order for ultra-fast monitoring:")
                Log.d(TAG, "  Order ID: $followOrderId")
    ...
    fun removeFollowOrderFromMonitoring(followOrderId: String) {
        scope.launch {
            monitoringMutex.withLock {
                activeFollowOrders.remove(followOrderId)
                processedResults.remove(followOrderId)
                Log.d(TAG, "Removed Follow Order from monitoring: $followOrderId")
            }
        }
    }
        ...
        monitoringJob = scope.launch {
            while (isMonitoringActive) {
                try {
                    ...
                    val ordersToCheck = mutableListOf<FollowOrderMonitoringState>()
                    val ordersToComplete = mutableListOf<String>()

                    monitoringMutex.withLock {
                        activeFollowOrders.values.forEach { orderState ->
                            when {
                                orderState.isCompleted -> {
                                    ordersToComplete.add(orderState.followOrderId)
                                }
                                currentTime - orderState.executionTime > FOLLOW_ORDER_TIMEOUT -> {
                                    Log.w(TAG, "Follow Order timeout: ${orderState.followOrderId}")
                                    handleFollowOrderTimeout(orderState)
                                    ordersToComplete.add(orderState.followOrderId)
                                }
                        ...
                        ordersToComplete.forEach { orderId ->
                            activeFollowOrders.remove(orderId)
                        }
                    }

                    if (ordersToCheck.isNotEmpty()) {
                        checkFollowOrdersViaAPI(ordersToCheck)
                    }
                    ...
                    Log.e(TAG, "Error in Follow Order monitoring loop: ${e.message}", e)
                    delay(1000L)
                }
            }
        }
    ...
    private suspend fun preWarmAPIConnections() {
        ...
        try {
            ...
            if (demoResponse.isSuccessful) {
                if (!isPreWarmed) {
                    isPreWarmed = true
                    Log.d(TAG, "Follow Order API pre-warming completed")
                }
            }
    ...
    private suspend fun checkFollowOrdersViaAPI(ordersToCheck: List<FollowOrderMonitoringState>) {
        ...
        try {
            ...
            Log.e(TAG, "Error checking follow orders via API: ${e.message}", e)
        }
    }

    private suspend fun checkOrdersForAccountType(
        orders: List<FollowOrderMonitoringState>,
        accountType: String,
            ...
            if (response.isSuccessful && response.body() != null) {
                ...
                orders.forEach { orderState ->
                    if (!orderState.isCompleted) {
                        val relevantTrade = findRelevantTradeForFollowOrder(historyData, orderState)
                        if (relevantTrade != null) {
                            processFollowOrderResult(orderState, relevantTrade, "API_DETECTION")
                        }
                    }
                }
            }
    ...
    private fun findRelevantTradeForFollowOrder(
        historyData: List<TradingHistoryRaw>,
        orderState: FollowOrderMonitoringState
    ): TradingHistoryRaw? {
        val searchWindowStart = orderState.executionTime - 15000L
        val searchWindowEnd = orderState.executionTime + 300000L
        val processedTradeId = processedResults[orderState.followOrderId]
    ...
    private suspend fun processFollowOrderResult(
        orderState: FollowOrderMonitoringState,
        ...
        monitoringMutex.withLock {
            ...
            processedResults[orderState.followOrderId] = trade.uuid

            activeFollowOrders[orderState.followOrderId] = orderState.copy(
            ...
            Log.d(TAG, "Follow Order result detected ($detectionMethod):")
            Log.d(TAG, "  Order ID: ${orderState.followOrderId}")
            ...
            onFollowTradeResult(orderState.followOrderId, isWin, resultDetails)
        }
    }
                ...
                when (event) {
                    "closed", "deal_result", "trade_update" -> {
                        ...
                        if ((tradeId.isNotEmpty() || uuid.isNotEmpty()) &&
                            ...
                            processWebSocketFollowResult(tradeId, uuid, status, amount, trend, payload)
                        }
                    }
                    "balance_changed" -> {
    ...
    private suspend fun processWebSocketFollowResult(
        tradeId: String,
        uuid: String,
        status: String,
        amount: Long,
        ...
        monitoringMutex.withLock {
            val matchingOrderId = activeFollowOrders.entries.find { (_, orderState) ->
                !orderState.isCompleted &&
            ...
            if (matchingOrderId != null) {
                val orderState = activeFollowOrders[matchingOrderId] ?: return@withLock
                ...
                activeFollowOrders[matchingOrderId] = orderState.copy(
                    isWebSocketDetected = true,
                    isCompleted = true
                )

                Log.d(TAG, "Follow Order WebSocket result (ULTRA-FAST):")
                ...
                onFollowTradeResult(matchingOrderId, isWin, resultDetails)
            }
        }
    }
    ...
    private suspend fun handleBalanceUpdate(payload: JSONObject?) {
        ...
        if (abs(balanceChange) > significantThreshold) {
            ...
            val recentOrderId = activeFollowOrders.entries
                .filter { (_, state) ->
                    !state.isCompleted &&
            ...
            if (recentOrderId != null) {
                val orderState = activeFollowOrders[recentOrderId]!!
                val isWin = balanceChange > 0

                Log.d(TAG, "Follow Order balance detection:")
                ...
                activeFollowOrders[recentOrderId] = orderState.copy(
                ...
                onFollowTradeResult(recentOrderId, isWin, resultDetails)
            }
        }
    ...
    private suspend fun handleFollowOrderTimeout(orderState: FollowOrderMonitoringState) {
        Log.w(TAG, "Follow Order timeout: ${orderState.followOrderId}")
        ...
        onFollowTradeResult(orderState.followOrderId, false, resultDetails)
    }
    ...
    fun getActiveOrdersCount(): Int = activeFollowOrders.size

    fun getMonitoringStats(): Map<String, Any> {
        return mapOf(
            "is_active" to isMonitoringActive,
            "active_orders" to activeFollowOrders.size,
            ...
            "performance_mode" to "ULTRA_FAST_FOLLOW_ORDER"
        )
    }

    fun cleanup() {
        stopMonitoring()
        Log.d(TAG, "Follow Order continuous monitor cleaned up")
    }
}